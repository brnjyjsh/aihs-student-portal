    ...
    <form action="" id="academic-form">
        ...
        <div class="row">
            <div class="col-md-6 form-group">
                ...
                <select name="subject_id" id="subject_id" class="form-control form-control-sm form-control-border rounded-0 select2" required>
                    <option <?= !isset($subject_id) ? 'selected' : '' ?> disabled></option>
                    <?php 
                        $subjects = $conn->query("SELECT c.*, d.name as strand FROM `subject_list` c inner join `strand_list` d on c.strand_id = d.id where c.delete_flag = 0 and c.status = 1 ".(isset($subject_id) ? " or c.id = '{$subject_id}' " : "")." order by d.name asc, c.name asc ");
                        while($row = $subjects->fetch_assoc()):
                    ?>
                    <option value="<?=  $row['id'] ?>" <?= isset($subject_id) && $subject_id == $row['id'] ? "selected" : '' ?>><?= $row['strand']. " - " . $row['name'] ?></option>
                    <?php endwhile; ?>
                </select>
            </div>
            <div class="form-group col-md-6">
